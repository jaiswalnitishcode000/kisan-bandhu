...
const AdminPortal = () => {
  ...
  if (!user || user.role !== "admin") return (
    <div className="min-h-screen flex items-center justify-center"><p className="text-muted-foreground text-lg">Admin access required.</p></div>
  );
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  ...
                  {users.length === 0 && <tr><td colSpan={3} className="px-4 py-8 text-center text-muted-foreground">No users registered yet.</td></tr>}
                </tbody>
              </table>
            </div>
          </div>
        ...
        <div className="space-y-3">
          {listings.map((l) => (
            <div key={l.id} className="bg-card rounded-xl border border-border shadow-card p-4 flex flex-col sm:flex-row justify-between gap-3">
              <div>
                <h3 className="font-semibold text-foreground">{l.cropName} <span className="text-sm text-muted-foreground">by {l.farmerName}</span></h3>
                <p className="text-sm text-muted-foreground">{l.quantity} quintals • Base: ₹{l.basePrice} • {l.bids.length} bids • {l.status}</p>
              </div>
              <button onClick={() => { removeListing(l.id); toast.success("Listing removed"); }}
