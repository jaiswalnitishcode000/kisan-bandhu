  ...
  return (
    <div className="min-h-screen py-8">
      <div className="container mx-auto px-4">
        <ScrollReveal>
          <h1 className="text-3xl font-bold text-foreground mb-1">🛒 Buyer Dashboard</h1>
        ...
        {myBids.length > 0 && (
          <div className="mb-10">
            <h2 className="text-xl font-semibold text-foreground mb-4">My Bid History</h2>
              ...
              {myBids.map((listing) => {
                ...
                return (
                  <div key={listing.id} className="bg-card rounded-xl border border-border shadow-card p-4 flex flex-col sm:flex-row justify-between gap-3">
                    <div>
                      <h3 className="font-semibold text-foreground">{listing.cropName} <span className="text-sm text-muted-foreground">by {listing.farmerName}</span></h3>
                      <p className="text-sm text-muted-foreground">My best bid: ₹{myBid[0]?.amount.toLocaleString()} • Highest: ₹{highestBid.toLocaleString()}</p>
                    </div>
        ...
        {/* Available Crops */}
        <h2 className="text-xl font-semibold text-foreground mb-4">Available Crops</h2>
