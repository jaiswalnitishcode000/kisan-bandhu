  ...
  return (
    <div className="bg-card rounded-xl border border-border shadow-card overflow-hidden hover:shadow-kisan transition-shadow duration-300 group">
      ...
      <div className="p-4 space-y-3">
        <h3 className="font-semibold text-lg text-foreground">{listing.cropName}</h3>
        ...
        <div className="space-y-1.5 text-sm text-muted-foreground">
          ...
          <p className="flex items-center gap-2 font-medium text-foreground">
            Base: ₹{listing.basePrice.toLocaleString()}/quintal
          </p>
          {highestBid > 0 && (
            <p className="flex items-center gap-2 text-primary font-semibold">
              <TrendingUp className="w-3.5 h-3.5" /> Highest Bid: ₹{highestBid.toLocaleString()}
            </p>
          )}
        </div>
